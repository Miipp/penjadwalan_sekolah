...
class Penjadwalan_front extends MY_Controller
{
	...
	public function index()
	{
		...
		$this->data['content']  = 'penjadwalan_front';
		$ext_foot = [
		...
		$this->data['mata_kuliah']= $this->mata_kuliah_m->get();
	...
	public function create_data(){
		...
		if(empty($jadwal_hari) || empty($jadwal_masuk) || empty($jadwal_keluar) || empty($ruang_id) || empty($mk_id) || empty($dosen_id) || empty($jadwal_kelas) || empty($jadwal_semester) || empty($jadwal_sks)){
			...
			$cekJadwal = $this->db->query("SELECT * FROM `jadwal` WHERE jadwal_hari = '$hari' and ('$jadwal_masuk_c' >= jadwal_jam_masuk   && jadwal_jam_keluar<='$jadwal_keluar_c')")->result();
